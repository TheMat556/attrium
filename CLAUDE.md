...
bun run build        # vue-tsc -b && vite build && build:css → app/dist/ (REQUIRED: PHP reads the manifest)
...
All tests are Playwright, and nearly all of them are visual regression (see below). Two CI workflows gate merge: `.github/workflows/ci.yml` runs `lint:check`, PHPCS, and a production build; `.github/workflows/visual.yml` runs the Playwright suite and fails on any pixel diff over threshold.
...
### Theme
`Attrium\Utility\Theme` owns the light/dark decision, and only it: a synchronous inline script printed in `<head>` on `admin_head` (priority 0, gated by the same `is_overlay_screen()` check as `load_styles()`, so the signal class and the token stylesheet are always present or absent together) and on `customize_controls_head`. It reads the `attrium-theme` localStorage key (absent or `'auto'` follows `prefers-color-scheme`; `'light'`/`'dark'` override), toggles `attrium-dark` on `<html>`, and owns the two runtime inputs — the `storage` event and the media query's `change` event. It must be inline PHP, not bundle TS: it has to run before first paint (a deferred module would flash light), and customize.php loads no Attrium bundle at all. `src/composables/useTheme.ts` is the shell's view, not a resolver: a `MutationObserver` on the carrier mirrors it to `#attrium-host` as `dark` — CSS variables do not cross the shadow-root boundary, and `src/style.css`'s `@custom-variant dark (&:is(.dark *, :host(.dark) *))` keys every `dark:` utility in the Vue components on that host class. `toggle()` writes only the explicit values, so `'auto'` now means "no choice stored" — it is only ever the absence of a key.

**`attrium-dark` on `<html>` is a signal, not a token carrier — do not widen it.** A dark carrier must cover exactly the surface Attrium reskins, and on a normal admin page that is the host subtree, not the document. Core appends fixed-position overlays straight to `<body>`, outside `#attrium-host`, and they keep core's own light chrome — but the module gates are descendant selectors (`body.attrium-mod-x &`), so Attrium's recipes still match inside them. Hanging the dark palette off `<html>` therefore renders dark text and buttons on a white core dialog; measured on theme-editor.php's `#file-editor-warning`, which the `theme-editor-dark` baseline captures because it is fixed-position and lands inside the host's bounding box (upload.php's media modal is the same shape with no baseline). So `scss/_tokens.scss` scopes the html-level carrier to `html.attrium-dark body.wp-customizer` — customize.php is the one page reskinned whole and the one with no host to hang the palette on — and `scss/modules/_forms.scss`'s chevron arm is scoped the same way for the same reason.
...
Nearly all tests are Playwright visual-regression snapshots that guard the SCSS/theme work (`tests/visual/`). They screenshot each restyled wp-admin screen with Attrium active and pixel-diff it against a committed baseline. The one exception is `theme-resolution.spec.ts`, which asserts behavior instead of pixels (see below).
...
- **Auth is one-shot.** `auth.setup.ts` logs in once and saves the session to `tests/visual/.auth/state.json`; both capture projects reuse it (a shared `setup` dependency, deduplicated by Playwright so it runs once per suite).
- **One config, two projects, parallel workers.** `playwright.config.ts` defines `screens` (full-page captures) and `components` (element captures under `tests/visual/components/`). They differ **only** in `expect.toHaveScreenshot.maxDiffPixels` (200 vs 20 — see below). This was previously two config files invoked in sequence with a shared `pw-base.ts` keeping them byte-identical; that failure mode is gone. Snapshot filenames come from an explicit `snapshotPathTemplate` with `-chromium` hardcoded, so renaming a project no longer invalidates all 132 baselines.
- **`workers: 4` and `timeout: 60_000` are pinned, not defaults.** Parallel is safe because the suite is read-only apart from the mu-plugin fixtures, which are seeded behind an atomic `add_option` claim; verified on a freshly destroyed instance. The worker count can't be left to Playwright because `os.cpus()` inside the container reports the host's 12 hyperthreads rather than the 4 CPUs the container gets, so its default (50%) resolves to **6** workers on a 4-core box — which blows the 30s default timeout and fails the run. Measured full-suite wall time: 5.2m at 1 worker, 2.7m at 2, 2.0m at 4; 6 is not faster even with the timeout raised. 4 also matches GitHub's `ubuntu-latest` runner for public repos.
...
- **`growViewportToFit` waits for the measured height to SETTLE, not to exceed the viewport.** Two consecutive equal readings is the signal that layout finished. The "taller than the viewport" condition it replaced could never be satisfied by a screen whose content genuinely fits in 900px, so those captures polled the full 5s timeout and then warned about a crop that had not happened — on 32 of 48 captures, ~2.5 min of pure sleep per run.
...
- **`theme-resolution.spec.ts` is the one behavioral spec — no screenshots, no baselines.** It pins the ONE resolver's output across both surfaces plus the shell's mirror by reading the carrier classes directly (`html.attrium-dark` set by `Attrium\Utility\Theme` on any page, `#attrium-host.dark` mirrored by the shell's MutationObserver, and the Customizer where only the html carrier exists — no host to mirror onto) for the states nothing else covers: `attrium-theme` absent and `auto`, each against `prefers-color-scheme` light and dark. The explicit `light`/`dark` values are already covered on both surfaces by `screens.spec.ts` and `customize.spec.ts`. **`applyTheme(page, null)` REMOVES the key rather than skipping the seed**, which is load-bearing: `auth.setup.ts` lands on wp-admin before saving `storageState`, so `attrium-theme: auto` is baked into `.auth/state.json` and restored into every context — `addInitScript` re-runs per navigation, so removal re-establishes "absent" on each hop.
