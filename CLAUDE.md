...
- **Captures target `#attrium-host`, and `fullPage` does not work.** The host is `position: fixed; inset: 0; overflow-y: auto`, so the *document* never scrolls — `fullPage: true` silently produced 1440×900 shots that cropped everything below the fold. Snapshotting the host alone isn't enough either (an element screenshot uses the bounding box, viewport-sized for a fixed element), so `snapshotTarget()` grows the viewport to the host's `scrollHeight` first. Baselines therefore vary in height (themes.php is 2318px tall).
