...
- **`theme-resolution.spec.ts` is the one behavioral spec — no screenshots, no baselines.** "Is the theme dark?" is answered twice, in two languages, and the split is inherent: `src/composables/useTheme.ts` (vueuse `useColorMode`) is the shell's reactive owner but ships in the bundle and so cannot run pre-paint, while `CustomizerSupport::theme_script()` must be a synchronous inline script because customize.php has no shell at all. The spec pins the two together by reading the carrier classes directly (`#attrium-host.dark` vs `html.attrium-dark`) for the states nothing else covers: `attrium-theme` absent and `auto`, each against `prefers-color-scheme` light and dark. The explicit `light`/`dark` values are already covered on both surfaces by `screens.spec.ts` and `customize.spec.ts`. **`applyTheme(page, null)` REMOVES the key rather than skipping the seed**, which is load-bearing: `auth.setup.ts` lands on wp-admin before saving `storageState`, so `attrium-theme: auto` is baked into `.auth/state.json`, and `useColorMode`'s `useStorage` writes its default back on every visit — `addInitScript` re-runs per navigation, so removal re-establishes "absent" on each hop.
