...
- `scss/ui/_primitives.scss` holds **element recipes** (pure style, include inside a gated rule: `link-primary`, `card-surface`, `icon-button`, `accent-hover`, …). `scss/ui/_patterns.scss` holds **composite recipes** that emit their own top-level blocks (`filter-toolbar`, `filter-drawer`, `upload-card`, `hide-bottom-bulk-actions`, …) — include those at top level under `screen-root(...)`, never `screen(...)`. The namespace (`primitives.` vs `patterns.`) is the enforcement of that rule.
- `scss/_mixins.scss` is gates only (`module`, `screen`, `screen-root`) plus the `dark` carrier mixin that emits both `#attrium-host.dark` and `html.attrium-dark` blocks. Keep style recipes out of it.
- The Customizer reskin is four files aligned with core's CSS structure: `screens/_customize-shell.scss` (overlay chrome + sidebar + footer), `_customize-controls.scss` (accordion + controls + editor), `_customize-themes.scss`, `_customize-menus.scss`.
